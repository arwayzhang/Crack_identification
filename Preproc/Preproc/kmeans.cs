...
namespace emgucv_image
{
    class kmeans
    {
        public double[] Runkmeans(double[][] rawData, int numCentroids)
        {
            ...
            double[] avg = Cluster(rawData, numCentroids);
            ...
            //ShowClustered(rawData, clustering, numCentroids);
        ...
        public static double[] Cluster(double[][] rawData, int numCentroids)
        {
            double[][] data = rawData;
            ...
            double[] avg = Averagedistance(data, clustering, means);
        ...
        private static double[] Averagedistance(double[][] data, int[] clustering, double[][] means)
        {
            var numCentroids = means.Length;
            //bool changed = false;

            int[] newClustering = new int[clustering.Length];
            double[] mindistances = new double[clustering.Length];
            Array.Copy(clustering, newClustering, clustering.Length);
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                double MinDistance = GetMinDistance(distances); // Find closest mean ID

                //Console.WriteLine(MinDistance);

                var newCentroidID = GetIndexOfMinDistance(distances); // Find closest mean ID

                //Console.WriteLine(newCentroidID);
            ...
            //Console.WriteLine(mindistances[100]);
            ...
            // Check proposed clustering[] cluster counts
            int[] clusterCounts = new int[numCentroids];
            double[] clusterSum = new double[numCentroids];
            for (int i = 0; i < data.Length; ++i)
            {
                int cluster = newClustering[i];
                ++clusterCounts[cluster];

                double mindis = mindistances[i];
                clusterSum[cluster] += mindis;
            }


            double[] avgdistance =new double[numCentroids];

            for (int k = 0; k < numCentroids; ++k)
            {
                if (clusterCounts[k] != 0)
                {
                    avgdistance[k] = clusterSum[k] / clusterCounts[k];
                }
                else
                {
                    avgdistance[k] = 0;
            ...
            //Console.WriteLine("=================");
            //Console.WriteLine(avgdistance[0]);
            //Console.WriteLine(clusterCounts[0]);
            //Console.WriteLine("=================");
        ...
        private static double GetMinDistance(double[] distances)
        {
            // Get index of smallest value in array
            int indexOfMin = 0;
            double smallDist = distances[0];
        ...
        static void ShowClustered(double[][] data, int[] clustering, int numCentroids)
        {
            for (int k = 0; k < numCentroids; k++)
            {
                Console.WriteLine("==========================================");
